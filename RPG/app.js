...
function play() {
  ...
  const options = readline.keyIn(
    " do you want to (w)alk or (p)rint player info? \n"
  );

  if (options === "p") {
    console.log(
      `\n Name: ${player.name} \n temper level: ${player.temper}\n Inventory: ${player.inventory} \n`
    );
    play();
  }
  if (options === "w") {
    ...
    while (enemyHealth > 0 && player.temper > 0) {
      ...
      if (choice === 0) {
        ...
        if (chanceOfAttack >= 2) {
          ...
          console.log(
            "This has turned into a battle and you're starting to lose your temper, but you won't go down without a fight"
          );
        } else console.log("you escaped! Keep moving");
      }
      if (choice === 1) console.log(`you hit with ${playerPower} power`);
